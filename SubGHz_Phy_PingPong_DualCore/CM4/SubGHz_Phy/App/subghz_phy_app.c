...
void sendCommand(data_type dt, int value)
{
	uint8_t hum_buf[]={0x68,0x75,0x6D,0x2E,0x76,0x61,0x6C,0x3D,0x30,0x30,0xff,0xff,0xff};
	uint8_t temp_buf[]={0x74,0x65,0x6D,0x70,0x2E,0x76,0x61,0x6C,0x3D,0x30,0x30,0xff,0xff,0xff};
	uint8_t rssi_buf[]={0x72,0x73,0x73,0x69,0x2E,0x76,0x61,0x6C,0x3D,0x30,0x30,0xff,0xff,0xff};
	switch(dt)
	{
		case humidity:
		{
			hum_buf[8]+=value/10;
			hum_buf[9]+=value%10;
			HAL_UART_Transmit(&huart1, (uint8_t *)hum_buf, sizeof(hum_buf),100);
		}
		break;
		case temperature:
		{
			temp_buf[9]+=value/10;
			temp_buf[10]+=value%10;
			HAL_UART_Transmit(&huart1, (uint8_t *)temp_buf, sizeof(temp_buf),100);
		}
		break;
		case rssi_dt:
		{
			rssi_buf[9]+=value/10;
			rssi_buf[10]+=value%10;
			HAL_UART_Transmit(&huart1, (uint8_t *)rssi_buf, sizeof(rssi_buf),100);
		}
		break;
		default:
		break;
...
bool receivedDisplayMessage = true;
...
static void OnRxDone(uint8_t *payload, uint16_t size, int16_t rssi, int8_t LoraSnr_FskCfo)
{
  ...
  snprintf(buf, sizeof(buf), (uint8_t *)"Rx Temp = %d Rx Humidity = %d\n", config.temperature, config.humidity);
...
static void PingPong_Process(void)
{
  ...
  switch (State)
  {
    case RX:
	  ...
	  {
		  config.humidity = getRandom(0);
		  config.temperature = getRandom(1);
		  snprintf(txBuf, sizeof(txBuf), (uint8_t *)"Tx Temp = %d Tx Humidity = %d\n", config.temperature, config.humidity);
		  APP_LOG(TS_ON, VLEVEL_L, txBuf);
		  memcpy(BufferTx, &config, sizeof(config));
		  Radio.Send(BufferTx, PAYLOAD_LEN);
	  }
...
void clearNewMsg()
{
	  sendCommand(temperature, config.temperature);
	  sendCommand(humidity, config.humidity);
	  sendCommand(rssi_dt, RssiValue);
...
//	{
//		APP_LOG(TS_ON, VLEVEL_L, "button press rx\n\r");
//	}
//}
//static void PingPong_Process(void)
//{
